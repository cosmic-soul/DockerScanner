...
class DockerTUI:
    ...
    def __init__(self, root: py_cui.PyCUI, demo_mode: bool = False):
        """Initialize Docker TUI.

        Args:
            root: The root py_cui window
        ...
        # Create health report widget
        self._create_health_report_widget()
    ...
    def _show_current_view(self):
        ...
        all_widgets = {
            ...
            "health_report_widget": self.health_report_widget,
        ...
        visible_widgets = {
            ...
            "health": ["health_report_widget"], #Only show the health report widget in the health view.
        ...
        if self.current_view == "overview":
            ...
            self.root.move_focus(self.health_report_widget)
        ...
        # Hide all widgets except active ones
        for widget_name, widget in all_widgets.items():
            widget.hidden = widget_name not in active_widgets
            
        # Update widgets only when the view is active.
    ...
    def _create_health_report_widget(self):
        """Create the health report widget."""
        self.health_report_widget = self.root.add_text_block(
            "System Health Report",
            1, 0, 4, 6  # Spans most of the window
        )
        self.health_report_widget.hidden = True

    def _show_health_report(self):
        """Display health report in TUI format."""
        try:
            # Create health report instance
            report = HealthReport(demo_mode=self.demo_mode)
            ...
            # System Status
            system = report.report_data.get("system", {})
            docker = report.report_data.get("docker", {})

            # Basic system info
            lines.append(f"CPU Usage: {system.get('cpu', {}).get('percent', 0)}%")
            lines.append(f"Memory Usage: {system.get('memory', {}).get('percent', 0)}%")
            lines.append(f"Disk Usage: {system.get('disk', {}).get('percent', 0)}%")

            # Docker status
            lines.append("\n=== Docker Status ===")
            lines.append(f"Status: {docker.get('status', 'unknown')}")
            if docker.get('containers'):
                containers = docker['containers']
                lines.append(f"Containers: {containers.get('total', 0)} total, "
                           f"{containers.get('running', 0)} running")

            # Recommendations
            lines.append("\n=== Recommendations ===")
            for rec in report.report_data.get("recommendations", []):
            ...
            # Set the content
            self.health_report_widget.set_text("\n".join(lines))

        except Exception as e:
            self.health_report_widget.set_text(f"Error generating health report: {e}")
    ...
    def _handle_quick_action(self):
        ...
        if selected_action == 0:  # Start Docker Service
            ...
            self.root.set_selected_tab(2)  # Switch to Health tab
            self._generate_health_report()
